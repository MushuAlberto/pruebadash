...
if uploaded_file is not None:
    ...
    def load_data(file):
        ...
        # Limpiar espacios en PRODUCTO, DESTINO, EMPRESA DE TRANSPORTE
    ...
    selected_date = st.date_input(
        ...
        fechas_disponibles[-1].date(),
        min_value=fechas_disponibles[0].date(),
        max_value=fechas_disponibles[-1].date()
    )

    # --- FILTRADO DE DATOS ---
    df_hoy = df[df['FECHA'] == pd.to_datetime(selected_date)]
    fecha_ayer = pd.to_datetime(selected_date) - timedelta(days=1)
    ...
    ultimos_7 = df[df['FECHA'] >= pd.to_datetime(selected_date) - timedelta(days=6)]
    line_df = ultimos_7.groupby(['FECHA', 'DESTINO'])['TONELAJE'].sum().reset_index()
    fig_line = px.line(
        line_df,
        x='FECHA',
        y='TONELAJE',
